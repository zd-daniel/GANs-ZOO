...
def parse_arg():
    ...
    parser.add_argument('--channels', default="32, 64, 128", type=str,
    ...
    parser.add_argument('--input_height', type=int, default=32, help='input image size')
    parser.add_argument('--input_width', type=int, default=None, help='input image size')
    parser.add_argument('--output_height', type=int, default=32, help='output image size')
...
def main(datatype='mnist', mode='ACGAN'):
    ...
    # -----------------load dataset--------------------------
    if datatype == 'mnist':
        config.dataroot = '/media/d/zhangdan/dataset/mnist/'
    elif datatype == 'cifar10':
        config.dataroot = '/media/d/zhangdan/dataset/cifar/'
    elif datatype == 'celebAHQ':
        config.dataroot = '/media/d/zhangdan/dataset/celebAHQ/celeba-64/'
    trainset, trainloader, testset, testloader, classes = \
        load_data(config.dataroot, datatype=datatype, batch_size=config.batch_size, output_size=config.output_height)
    ...
    if mode == 'ACGAN':
        ...
        model = CGAN(cdim=3, hdim=config.noise_dim,
                     channels=str_to_list(config.channels), image_size=config.output_height,
                     class_num=10, is_mnist=False).to(device)
...
if __name__ == "__main__":
    main(datatype='cifar10', mode='CGAN')
