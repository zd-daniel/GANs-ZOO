...
from CGAN.CustomLayers import GenInitialBlock, GenGeneralResBlock, GenFinalResBlock, \
...
class Discriminator(nn.Module):
    def __init__(self, cdim=3, hdim=512, channels=[64, 128, 256, 512, 512, 512],
        ...
        self.layers.append(DisInitialBlock(cdim, out_channels=cc, feature_size=sz, class_num=class_num))
        for ch in channels[1:]:
            self.layers.append(DisGeneralResBlock(cdim, cc, ch, sz // 2, class_num=class_num))
            cc, sz = ch, sz // 2
...
class CGAN(nn.Module):
    ...
    def discriminate(self, x, label):
        logits = data_parallel(self.dis, (x, label))
        return logits
